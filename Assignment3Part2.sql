DECLARE
    ...
    V_TRANSACTION_DEBITS        NUMBER := 0;
    V_TRANSACTION_CREDITS       NUMBER := 0;
...
BEGIN
    ...
    FOR R_TRANSACTIONS IN C_TRANSACTIONS LOOP
        ...
        BEGIN
            ...
            SELECT
                COUNT(*) INTO V_ACCOUNT_NO_EXISTS
            FROM
                ACCOUNT
            WHERE
                ACCOUNT_NO = R_TRANSACTIONS.ACCOUNT_NO;
            IF V_ACCOUNT_NO_EXISTS = 0 THEN
                V_TRANSACTION_ERROR_MESSAGE := 'Acount ID is not found: '
            ...
            IF R_TRANSACTIONS.TRANSACTION_AMOUNT < 0 THEN
                V_TRANSACTION_ERROR_MESSAGE := 'Transaction amount is negative: '
            ...
            IF R_TRANSACTIONS.TRANSACTION_TYPE NOT IN (K_CR, K_DR) THEN
                V_TRANSACTION_ERROR_MESSAGE := 'Invalid transaction type: '
                                               || R_TRANSACTIONS.TRANSACTION_TYPE;
                RAISE E_INVALID_TRANSACTION;
            END IF;
        EXCEPTION
            ...
            WHEN E_INVALID_TRANSACTION THEN
                INSERT INTO WKIS_ERROR_LOG (
                    ...
                    V_TRANSACTION_DEBUG_ROW.TRANSACTION_NO,
                    V_TRANSACTION_DEBUG_ROW.TRANSACTION_DATE,
                    V_TRANSACTION_DEBUG_ROW.DESCRIPTION,
                    V_TRANSACTION_ERROR_MESSAGE
                );
                DELETE FROM NEW_TRANSACTIONS WHERE TRANSACTION_NO = V_TRANSACTION_DEBUG_ROW.TRANSACTION_NO;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error in Transaction No: '
                             || V_TRANSACTION_DEBUG_ROW.TRANSACTION_NO
                             || ' Type: '
                             || V_TRANSACTION_DEBUG_ROW.TRANSACTION_TYPE
                             || ' - '
                             || SQLERRM);
        ROLLBACK;
END;
